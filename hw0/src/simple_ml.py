...
def parse_mnist(image_filename, label_filename):
    ...
    with gzip.open(image_filename,'rb') as f:
        image_bin = f.read()
        magic_number_image,image_count,rows, cols = struct.unpack('>IIII',image_bin[0:16])
        image_array_raw = np.frombuffer(image_bin,dtype=np.uint8,offset=16)
        image_array = (image_array_raw.astype(np.float32) / (255)).reshape(image_count,rows*cols)

    with gzip.open(label_filename, "rb") as f:
        label_bin = f.read()
        magic_number_label,label_count = struct.unpack('>II',label_bin[0:8])
        label_array = np.frombuffer(label_bin,dtype=np.uint8,offset=8)
...
def softmax_loss(Z, y):
    ...
    ### BEGIN YOUR CODE
    Z_exp = np.exp(Z)
    exp_sum = np.sum(Z_exp,axis=1)
    log_sum = np.log(exp_sum)
    correct_score = Z[np.arange(Z.shape[0]),y]
    return np.mean(log_sum - correct_score)
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0,X.shape[0],batch):
        X_batch = X[i:min(i+batch,X.shape[0])] # n * input_dim
        y_batch = y[i:min(i+batch,X.shape[0])]
        batch_size = X_batch.shape[0]
        logits = np.exp(X_batch @ theta) # n * k
        Z = logits / np.sum(logits,axis=1,keepdims=True) # n * k
        Z[np.arange(batch_size),y_batch] -= 1
        grad = (X_batch.T @ Z) / batch_size
        theta -= grad * lr # input_dim * k
    ### END YOUR CODE
